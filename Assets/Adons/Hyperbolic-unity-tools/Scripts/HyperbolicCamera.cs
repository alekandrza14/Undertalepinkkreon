...
public class HyperbolicCamera : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        {
          HyperbolicPoint hbpos = Instantiate(DevPrefab,transform.position,Quaternion.identity).GetComponent<HyperbolicPoint>();
            hbpos.HyperboilcPoistion = RealtimeTransform.copy().inverse();
        }
#endif
        ...
        //Vertical
            ...
            Vector3 v = Speed * Vector3.MoveTowards(Vector3.zero, new Vector3(-Input.GetAxis("Vertical") * Time.deltaTime, -Input.GetAxis("Horizontal") * Time.deltaTime), 2) ;

            RealtimeTransform.preApplyTranslationZ(v.x);
        RealtimeTransform.preApplyTranslationY(v.y);
        RealtimeTransform.preApplyRotation(-Input.GetAxis("Vertical2") * Time.deltaTime*3);
         RealtimeTransform.preApplyRotation((Input.GetKey(KeyCode.Keypad4) ? 1 : 0) * Time.deltaTime * 3);
        RealtimeTransform.preApplyRotation((Input.GetKey(KeyCode.Keypad6) ? -1 : 0) * Time.deltaTime * 3);
